...
(* call parser to parse input expression *)
...
(* evaluated, pauses for user input, performs a reduction, and then        *)
(* prints out the result.                                                  *)
let rec cbv (e : expr) : expr =
  if is_value e then
    (print_string "Result: "; print_endline (to_string e); e)
  else
    (print_endline (to_string e); print_string ">>"; ignore (read_line()); cbv (cbv_step e))
